<template>
  <div class="rates">
    <SectionHeader class="text-primary"> Список товаров </SectionHeader>

    <button
      @click="addRate"
      class="ml-auto mt-2 btn btn-success d-flex justify-content-between align-items-center gap-2"
    >
    ...
    <table class="table table-striped table-hover mt-2">
      ...
      <tbody v-if="rates.length">
        <tr
          ...
          :key="rate._id"
          draggable="true"
          @dragstart="onDragStart(rate, index)"
          @dragover.prevent
          @drop="onDrop(rate, index)"
        >
          ...
          <td class="border col-3">
            {{
              rate.body.length > 60 &&
              !ratesMore.find(more => more.id == rate._id).more
                ? rate.body.slice(0, 60)
                : rate.body
            }}

            <div class="w-auto d-flex justify-content-center mt-1">
              <button
                @click="seeMore(rate._id)"
                class="rate-more btn btn-secondary"
                v-if="
                  rate.body.length > 50 &&
                  !ratesMore.find(more => more.id == rate._id).more
                "
              ...
              <button
                @click="hideRateBody(rate._id)"
                class="rate-more btn btn-secondary"
                v-if="
                  ratesMore.find(more => more.id == rate._id).more && rate.body
                "
              >
                Скрыть
          ...
          <td class="border text-center align-middle">{{ rate.price }} ₽</td>
          <td class="border text-center align-middle">
            {{ rate.link }}
          </td>
          <td class="border col-2 align-middle">
            <div
              class="d-flex m-auto justify-content-center align-items-center gap-2"
            >
              <button
                class="btn btn-light btn-sm d-flex"
                @click="changeRate(rate._id)"
              >
                Редактировать
              </button>

              <button
                class="btn btn-danger btn-sm d-flex gap-1"
                @click="removeRate(rate._id)"
              >
                Удалить
              </button>
            </div>
    ...
    <tr
      v-if="!rates.length && !$store.getters.getIsLoading"
      class="col-12 text-center fs-3 d-flex justify-content-center"
    >
