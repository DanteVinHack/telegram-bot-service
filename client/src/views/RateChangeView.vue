<template>
  <form @submit='changeRate' class='rate container-sm bg-white'>
    ...
    <h2 class="mt-5 fs-3 text-secondary">
      Название
    </h2>
    ...
    <div class="rate__wrapper mt-3 d-flex gap-3">
      ...
      <button type='button' class="btn btn-light w-auto">
        Отменить
      </button>
  ...
  methods: {
    async changeRate(e) {
      try {
        ...
        await this.axios.post('http://localhost:4000/rate', this.rate)
    ...
    async getRate() {
      const response = await this.axios.get(`http://localhost:4000/rate/${this.rate._id}`, {_id: this.id})
